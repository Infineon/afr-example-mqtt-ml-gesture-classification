...
* File Name:   main.c
*
* Description: This is the source code for CE233694 - AWS IoT and 
*              FreeRTOS for PSoC® 64 MCU: MQTT ML Gesture Classification
*
...
void vApplicationDaemonTaskStartupHook( void )
{
    ...
    if( SYSTEM_Init() == pdPASS )
    {
        ...
        printf("Device provisioning failed with error code %ld\r\n", xResult);
#endif
        
        /* Initialize the motion sensor on the TFT shield */
        result = sensor_init();
        if(CY_RSLT_SUCCESS != result)
        {
            ...
            /* Reset the system on sensor failure */
            NVIC_SystemReset();
        }
...
WIFIReturnCode_t prvWifiConnect( void )
{
    ...
    if( strlen(clientcredentialWIFI_SSID) > 0 )
    {
    ...
    if( xWifiStatus != eWiFiSuccess )
    {
        ...
        {
            xSSIDLength = strlen(clientcredentialWIFI_SSID);
            if(xSSIDLength <= wificonfigMAX_SSID_LEN)
            {
                xNetworkParams.ucSSIDLength = xSSIDLength;
                memcpy(xNetworkParams.ucSSID, clientcredentialWIFI_SSID, xSSIDLength);
            }
        ...
        {
            if( strlen(wificonfigACCESS_POINT_PASSKEY) > 0 )
            {
